...
```ts
  ...
  55 | 
  56 |     const rows = this.page.locator(pimLocators.employeeRows);
  57 | 
  58 |     const count = await rows.count();
  59 | 
  60 |     for (let i = 0; i < count; i++) {
  61 | 
  62 |         const text = await rows.nth(i).textContent();
  63 | 
  64 |         if (text.includes(employeeName)) {
  65 |             return true;
  66 |         }
  67 |     }
  68 | 
  ...
  70 | }
  71 | 
  72 | async getNoRecordsMessage() {
  73 | 
  74 |     await this.page.waitForSelector(pimLocators.noRecordsFound);
  75 | 
  76 |     return await this.page
  77 |         .locator(pimLocators.noRecordsFound)
> 78 |         .textContent();
     |          ^ Error: locator.textContent: Error: strict mode violation: locator('.oxd-text--span') resolved to 12 elements:
  79 | 
  80 | }
  81 | 
  82 |     async enterEmployeeId(employeeId) {
  83 |         await this.page.locator(pimLocators.employeeId).fill(employeeId);
  84 | }
  85 | }
  86 | 
  87 | module.exports = PimPage;
```