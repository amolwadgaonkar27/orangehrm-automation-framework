...
Call log:
  - waiting for locator('.oxd-select-dropdown') to be visible
...
```ts
  ...
  42 |         const options = await this.page.locator('.oxd-select-option').allTextContents();
  43 |             console.log(options);
> 44 |         await this.page.waitForSelector('.oxd-select-dropdown');
     |                         ^ Error: page.waitForSelector: Test timeout of 60000ms exceeded.
