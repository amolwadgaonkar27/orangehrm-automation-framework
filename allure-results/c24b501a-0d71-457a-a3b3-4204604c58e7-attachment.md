...
Call log:
  - waiting for locator('.oxd-select-option span').filter({ hasText: 'Bug Fixes' })
...
```ts
  ...
  36 |         await this.page.locator('.oxd-autocomplete-option').first().click();
  37 |         await this.page.waitForTimeout(1000);
  38 |         const value = await this.page.locator(timeLocators.projectInput).inputValue();
  39 |             console.log("Selected Project:", value);
  40 |         await this.page.locator(timeLocators.activityDropdown).click();
  41 |         const options = await this.page.locator('.oxd-select-option').allTextContents();
  42 |             console.log(options);
  43 |         await this.page.waitForSelector('.oxd-select-dropdown');
  44 |         await this.page.locator('.oxd-select-option span', {
  45 |             hasText: activity
> 46 |                 }).click();
     |                    ^ Error: locator.click: Test timeout of 60000ms exceeded.
  47 |         const mondayInput = this.page.locator(timeLocators.mondayCell).first();
  48 |         await mondayInput.waitFor({ state: 'visible' });
  49 |         await mondayInput.click();
  50 |         await mondayInput.fill(hours);
  51 |         await this.page.click(timeLocators.saveButton);
  52 |     }
